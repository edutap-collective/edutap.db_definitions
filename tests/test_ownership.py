...
def test_a_document_applies_to_a_database_that_has_none_of_its_schemas(
    ...
    """The `engine` fixture only resets `public` between tests, so `pass_builder`
    would otherwise survive into whatever runs next in the same session — a
    session-scoped container, per `postgres_url`. Harmless today, but an
    order-dependent failure waiting for the moment something reflects across
    schemas (Task 4). The `try/finally` drops it regardless of which assertion
    fails.
    """
    ...
    try:
        ...
        with engine.connect() as connection:
            found = connection.execute(
                text(
                    "SELECT table_schema FROM information_schema.tables WHERE table_name = 'thing'"
                )
            ).scalar_one()
